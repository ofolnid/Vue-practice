...
const handleCreate = async () => {
  ...
  try {
    ...
    items.value.unshift(response.data) // 배열 맨 앞에 응답 데이터 추가
    textInput.value = '' // 인풋 초기화
  } catch (error) {
...
const handleDelete = async (id) => {
  try {
    ...
    console.log('DELETE 요청 성공. 상태 코드: ', response.status)
...
// 컴포넌트 마운트 직후 get 요청을 바로 보내 데이터를 가져오기
onMounted(() => {
  handleRead()
})
</script>
